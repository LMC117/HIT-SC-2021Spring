...
public class parserInput {
     ...
     * @param str 输入字符串
     */
    private static void checkInput(String str) throws IOException {
        ...
        String filePath = "src/txt/test" + str + ".txt";
        parser(filePath);
    }

    private static void parser(String filePath) throws IOException {
        // 读取txt文件
        ...
        boolean flagPeriod = DSet.setDate(periodSplit[1], periodSplit[2]);
        if (!flagPeriod) {
        ...
        for (String roster : rosters) {
            ...
            boolean flagRoster = DSet.manualRosters(rosterSplit[0], rosterSplit[1], rosterSplit[2]);
            if (!flagRoster) {
