...
import ADT.Interval.NoBlankIntervalSet;
import ADT.Interval.NonOverlapIntervalSet;
...
public class DutyIntervalSet {
    ...
    private IntervalSet<Employee> schedule = new CommonIntervalSet<>(); // 保存时间段
    ...
    public boolean manualRosters(String name, String sStart, String sEnd) {
        ...
        if (checkInRoster(e)) { // 员工已在排班表中，不能再添加
            System.out.printf("员工%s已在排班表中！\n", name);
            return false;
        }
        ...
        // 检查是否存在日期重叠
        IntervalSet<Employee> intervalSet = new NonOverlapIntervalSet<>(schedule);
        if (intervalSet.checkOverlap()) { // 存在重叠
            schedule.remove(e);
    ...
    public boolean deleteRoster(String name, String start) {
        ...
        long begin = schedule.start(e);
        long end = schedule.end(e);
        ...
        List<Integer> date = dateConversion(begin, end);
        String[] split = start.split("-");
        for (int i = 0; i < split.length; i++) {
            if (!(date.get(i) == Integer.parseInt(split[i]))) {
    ...
    public void checkFullRoster() {
        IntervalSet<Employee> intervalSet = new NoBlankIntervalSet<>(schedule);

        Iterator<Employee> it = intervalSet.labels().iterator(); // 方便遍历标签集合
    ...
    public void rosterVisualization() {
        ...
        if (schedule.isEmpty()) {
            ...
            for (Employee e : schedule.labels()) {
                long eStart = schedule.start(e);
                long eEnd = schedule.end(e);
                List<Integer> list = dateConversion(eStart, eEnd);
    ...
    private Map<Long, Long> saveMap() {
        ...
        while (it.hasNext()) {
            Employee label = it.next();
            map.put(schedule.start(label), schedule.end(label));
        }
     ...
     * @return 计算得到的空闲时间
     */
    private double calcFreeTimeRatio(IntervalSet<Employee> set) {
        long blank = 0;
        ...
        while (it.hasNext()) {
            Employee label = it.next();
            map.put(set.start(label), set.end(label));
        }
